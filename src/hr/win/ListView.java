...
public class ListView extends JPanel implements javax.swing.event.AncestorListener{
	...
	private JPanel getPanOption() {
		if (panOption == null) {
			...
			panOption.setBorder(BorderFactory.createTitledBorder(null, "필드 옵션", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.TOP, new Font("Dialog", Font.BOLD, 12), new Color(51, 51, 51)));
		}
		return panOption;
	}
	...
	public void setData(List<? extends IMember> list){
		initData();
		//System.out.println(new MemberManager().getPlayers().size() + "컁컁");
		try{
		...
		catch(IndexOutOfBoundsException e){System.out.println("음?");}
		//System.out.println(new MemberManager().getPlayers().size() + "킁킁");
	}
